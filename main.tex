...
%
\begin{multline}
  L_{\mathrm{p}} = \dfrac{1}{2} \norm{\beta} + C \sum \limits_{i =1}^{P} (1 - \mu_i)\zeta_i \\
  - \sum \limits_{i =1}^{P} \alpha_i \left[ y_i \left( \phi(x_i)^T \beta + \beta_0 \right) - (1 - \zeta_i) \right].
  \label{eq:l_primal}
\end{multline}
%
By minimizing $L_{\mathrm{primal}}$ with respect to $\beta$, $\beta_0$, and $\zeta_i$, we get the corresponding dual form of the Lagrange function,
%
\begin{equation}
  L_{\mathrm{dual}} = \sum \limits_{i = 1}^{P} \alpha_i - \frac{1}{2} \sum \limits_{i = 1}^{P} \sum_{i' = 1}^{P} \alpha_i \alpha_{i'} y_i y_{i'} \langle \phi(x_i), \phi(x_{i'}) \rangle
  \label{eq:l_dual}
\end{equation}
%
...
%
\begin{equation}
  I(\mathbf{X}, \mathbf{Y}) = - \int p_i\ln p_i - \sum \limits_j p_j\ln p_j + \sum_j \int p_{ij}\ln p_{ij},
  \label{eq:MI}
\end{equation}
%
...
%
\begin{equation}
  R(\mathcal X) = \sum \limits_{\mathbf{X}_i, \mathbf{X}_j \in \mathcal{X}} I(\mathbf{X}_i, \mathbf{X}_j).
  \label{eq:R}
\end{equation}
%
...
\end{table}
%
In the second phase, we further pruned the number of features to four only by selecting the ones that provided the best performance in terms of validation accuracy using the SVM classification scheme. Table \ref{tab:best_features} shows the four best features obtained by averaging the validation accuracy over \num{100} iterations of the classification scheme.
%
\begin{table}[!htbp]
  \caption{Validation Performance of individual features in \num{100} iterations of running a SVM model with RBF kernel.}
  \sisetup{
  round-mode = places,
  round-precision = 3
  }%
...
%
%
In the second phase, we further pruned the number of features to four with the evaluation criteria of mean classification accuracy on the validation set. For this purpose, we ran the  only by selecting the ones that provided the best performance in terms of validation accuracy using the SVM classification scheme. Table sdf shows the four best features obtained by averaging the validation accuracy over \num{100} iterations of the classification scheme.
%
