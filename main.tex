...
\begin{abstract}
We present a predictive tool that determines the future risk of developing type 2 diabetes mellitus (T2DM) through machine learning techniques. Data generated from an oral glucose tolerance test (OGTT) was used to develop a predictive model based on the support vector machines. We trained and validated the models on the data collected from a group of 1,551 healthy subjects part of the San Antonio Heart Study. In this paper, we show that just by using the information extracted from a person’s abnormal blood glucose levels, high predictive performance can be obtained. The results of our approach show an average accuracy of 98.51\% and recall of 87.27\% obtained on a holdout set.
\end{abstract}
%
