...
For a linear variant of the SVM, 
%
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
...
%
In this paper, we employed the non-linear SVM in the form of radial basis functions (RBF) and also its linear variant, with the goal to observe the performance in the presence of inseparable data in terms of the class. Due to the highly imbalanced nature of the dataset, we constructed the models by first balancing the dataset and then retaining the class ratio. In the former case, we created a randomly selected training set of \num{320} samples containing equal instances of the diabetic and healthy classes. In the latter case, \num{1360} samples were used to generate the training set with \num{160} and \num{1200} instances of the diabetic and healthy classes respectively. During the model generation phase, we used \num{10}-fold cross-validation

Therefore, we took two approaches for the training set, one in which the dataset was balanced by a randomized undersampling of the majority class, and second in which we persisted with natural class distribution. In both the cases, the validation set had the same class distribution representative of the original dataset. A total of \num{1360} samples were used for training that accounted for \SI{91}{\percent} of the dataset, and \num{99} samples, consisting of \num{11} diabetic and \num{83} healthy subjects were reserved for the validation of the trained prediction models.
